...
def query_hotlist_two(meta_dd, pcid, pot_con, timestamp):
...
def annotate_and_respond(text):
    ...
    for pcid, potential_concept in potent_con.items():
        ...
        conceptdict2 = query_hotlist_two(meta_dd, pcid, potential_concept, timestamp_log)
...
annotate_and_respond("i really hate and love brundage very much in america")
...
#text = "i really hate angry bad envy"
#processor = subprocess.Popen(['echo {} | ./emotionStream.sh'.format(text)], stdout=subprocess.PIPE, shell=True)
#processor_output, _ = processor.communicate()
